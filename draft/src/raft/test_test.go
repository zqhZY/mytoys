...
func TestInitialElectionXA(t *testing.T) {
	servers := 3
...
func TestReElection2A(t *testing.T) {
	...
	fmt.Printf("\nnow dissconnect leader1: %d\n", leader1)
	
	// if the leader disconnects, a new one should be elected.
	cfg.disconnect(leader1)
	cfg.checkOneLeader()
	DPrintf("\nleader1: %d disconnected \n", leader1)
	// if the old leader rejoins, that shouldn't
	// disturb the old leader.
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()
	DPrintf("\nleader2 %d , now check no leader disconnect 2 server: \n", leader2)
	...
	DPrintf("\ncheck one node if 2 server connected \n")
	// if a quorum arises, it should elect a leader.
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()

	DPrintf("\n3 node alive , should one node \n")
