...
func setup() *MapReduce {
  file := makeInput()
  //master := port("master")
  mr := MakeMapReduce(nMap, nReduce, file, MasterAddress)
  return mr
}
...
func TestBasic(t *testing.T) {
  ...
  workerport := 8888
  for i := 0; i < 2; i++ {
	// Run Workres on localhost
    go RunWorker(mr.MasterAddress, "127.0.0.1:" + strconv.Itoa(workerport),
...
func TestOneFailure(t *testing.T) {
  ...
  go RunWorker(mr.MasterAddress, "127.0.0.1:" + strconv.Itoa(8883),
               MapFunc, ReduceFunc, 3)
  go RunWorker(mr.MasterAddress, "127.0.0.1:" + strconv.Itoa(8884),
               MapFunc, ReduceFunc, -1)
  // Wait until MR is done
  <- mr.DoneChannel
  check(t, mr.file)
...
func TestManyFailures(t *testing.T) {
  ...
  for !done {
    ...
    default:
      ...
      w := "127.0.0.1:" + strconv.Itoa(i)
      go RunWorker(mr.MasterAddress, w, MapFunc, ReduceFunc, 3)
      i++
      w = "127.0.0.1:" + strconv.Itoa(i)
      go RunWorker(mr.MasterAddress, w, MapFunc, ReduceFunc, 3)
      i++
