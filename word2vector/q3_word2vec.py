...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset,
    ...
    *                   *
    ...
    y_ = []  # save all output probability
    for indice in indices:
        if indice == target:
            y_.append(sigmoid(np.dot(predicted, outputVectors[indice].T)))
        else:
            y_.append(sigmoid(-np.dot(predicted, outputVectors[indice].T)))

    cost = -np.sum(np.log(y_))

    deta = np.array(y_)  # 1 x (K+1)
    deta[0] -= 1  # error of target
    deta[1:] = 1 - deta[1:] # error of negative samples

    grad = np.zeros_like(outputVectors)
    gradPred = np.zeros_like(predicted)

    for i in range(K+1):  # update all grads (one positive with K negative)
        grad[indices[i]] += deta[i] * predicted # 1 * N grad of output metric
        gradPred += outputVectors[indices[i]].T * deta[i]  # grad of predicted (middle layer vector)
